<script lang="ts">
	...
	$effect(() => {
		...
		if (htmlContent && markdownBody && !isEditing && hljs && renderMathInElement && mermaid) {
			...
			// Debug: check for math spans
			const mathSpansDebug = markdownBody.querySelectorAll('span[data-math-style]');
			console.log('[KaTeX Debug] Found', mathSpansDebug.length, 'math spans');
			mathSpansDebug.forEach((span, i) => {
				console.log(`[KaTeX Debug] Span ${i}:`, span.getAttribute('data-math-style'), span.textContent?.substring(0, 50));
			});
